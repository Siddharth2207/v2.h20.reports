...
import { tokenConfig, networkConfig } from "./constants";
import { settings } from "$lib/stores/report";
import { get } from "svelte/store";
import axios from "axios";
import type { LiquidityAnalysisResult, NetworkConfigSourceWithBlockTime, TokenConfig, TradesByTimeStamp } from "./types";
...
async function getBlockNumberForTimestampByBlockTime(
    ...
    const approxNearBlock = latestBlockNumber - Math.floor(network.blockTime ? diff / network.blockTime : 0);
...
async function analyzeHyperSyncData(
    ...
    let tradesAccordingToTimeStamp:TradesByTimeStamp[] = []
