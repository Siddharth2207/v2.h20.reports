...
export const getTokenOrders = async () => {
    ...
    const allOrders = await getOrders(get(activeSubgraphs), {
        owners: [],
        active: true,
    ...
    const tokenFilteredOrders = allOrders.filter(
        (order: any) =>
            order.order.inputs.some(
                (input: any) =>
                    input.token.symbol === tokenSymbol && input.token.address.toLowerCase() === tokenAddress.toLowerCase(),
            ) ||
            order.order.outputs.some(
                (output: any) =>
                    output.token.symbol === tokenSymbol && output.token.address.toLowerCase() === tokenAddress.toLowerCase(),
            )
    );
    ...
    for(let order of tokenFilteredOrders) {
        ...
        order.order["trades"] = trades.sort((a: any, b: any) => b.tradeEvent.transaction.timestamp - a.tradeEvent.transaction.timestamp);
    }
