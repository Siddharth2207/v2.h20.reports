<script lang="ts">
    ...
    import { DEFAULT_ORDERS_PAGE_SIZE, getContext, interpreterV3Abi, qualifyNamespace } from '$lib/constants';
    ...
    async function getOrderQuotes(orders: MarketDepthOrder[]): Promise<MarketDepthOrder[]> {
        ...
        const quoteSpecs: OrderQuoteValue[] = await doQuoteSpecs(orderBatchQuoteSpecs, $settings.subgraphs[network], networkRpc === '' || networkRpc === undefined ? $settings.networks[network].rpc : networkRpc);
    ...
    async function validateHandleIO(order: MarketDepthOrder): Promise<boolean> {
        ...
        const networkProvider = new ethers.providers.JsonRpcProvider(networkRpc === '' || networkRpc === undefined ? $settings.networks[network].rpc : networkRpc);
