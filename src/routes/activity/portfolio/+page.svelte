<script lang="ts">
	...
	import { fetchAllPaginatedData, isOrderDsf, isOrderPortfolio } from '$lib/orders';
	...
	function filterRaindexData(data: RaindexData[]): RaindexData[] {
		...
		filteredData = filteredData.filter((trade) => {
			...
			return isOrderPortfolio(trade.orderMeta);
		});
...
<div class="flex min-h-screen flex-col p-2 md:p-4">
	<div class="mb-3 flex flex-col items-center gap-2 md:mb-5 md:gap-4">
		<h2 class="text-lg font-bold text-gray-800 md:text-2xl">Raindex Portfolio Data</h2>
	</div>
	<div
